...
export default class FeaturedImage extends Plugin {
    ...
    public async importSettings(settingsText: string): Promise<void> {
        try {
            // Skip empty or comment-only input
            if (!settingsText.trim() || settingsText.trim().startsWith('#')) {
                new Notice('No settings to import');
                return;
            }

            const newSettings: Partial<FeaturedImageSettings> = {};
            ...
            settingsText.split('\n').forEach(line => {
                ...
                if (key && valueStr) {
                    ...
                    // Validate that the key exists in DEFAULT_SETTINGS
                    if (trimmedKey in DEFAULT_SETTINGS) {
                        try {
                            newSettings[trimmedKey as keyof FeaturedImageSettings] = JSON.parse(valueStr.trim());
                        } catch (e) {
                            console.error(`Failed to parse value for ${key}:`, e);
                        }
                    } else {
                        console.warn(`Unknown setting key: ${trimmedKey}`);
                    }
                }
            });

            // Only proceed if we have valid settings to import
            if (Object.keys(newSettings).length === 0) {
                new Notice('No valid settings found to import');
                return;
            }
